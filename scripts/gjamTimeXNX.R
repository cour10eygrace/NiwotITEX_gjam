...
#subset data for XNX plots
plots<-subset(sppcomp, code=="XNX")  
...
plothits_XNX<-filter(plothits, plot %in% plots$plot)
vals=plothits_XNX$veg_hits
...
modDAtimeXNX<- gjam(formula=timeList$formulaRho, xdata = xdata, ydata = ydata, modelList = modelList)

# save output
save(modDAtimeXNX, file = "C:/Users/court/Google Drive/CU Postdoc/LTREB/gjam/modDAtime_XNXoutput.RData")
...
plotPars <- list(specColor=specColor, PLOTALLY=T, GRIDPLOTS=T, CLUSTERPLOTS=T, SAVEPLOTS = T, 
                 outFolder = 'C:/Users/court/Google Drive/CU Postdoc/LTREB/gjam/modDAtime_XNXplots')
gjamPlot(modDAtimeXNX, plotPars)

#posterior_vs_prior(modDAtime)#how to plot this???

#spp alphas
alphaX<-modDAtimeXNX$parameters$alphaMu
colnames(alphaX)<-colnames(ydata)
row.names(alphaX)<-colnames(alphaX)  

pdf(file="C:/Users/court/Google Drive/CU Postdoc/LTREB/gjam/modDAtime_XNXplots/alpha_plot.pdf")
...
corr<-modDAtimeXNX$parameters$corMu
colnames(corr)<-colnames(ydata)
row.names(corr)<-colnames(corr)  

pdf(file="C:/Users/court/Google Drive/CU Postdoc/LTREB/gjam/modDAtime_XNXplots/corr_plot.pdf")
...
wstarXNX <- .wrapperEquilAbund(output =   modDAtimeXNX, covars = c('depthcm', 'avgT', 'Ndep'), BYGROUP = T,
                               nsim = 10, ngrid=10, 
                               verbose = T)
save(wstarXNX, file = "C:/Users/court/Google Drive/CU Postdoc/LTREB/gjam/wstar_XNXoutput.RData")
#load(file = "C:/Users/court/Google Drive/CU Postdoc/LTREB/gjam/wstar_XNXoutput.RData")
