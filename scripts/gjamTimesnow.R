...
snowplots<-subset(sppcomp, snow!="X")  
xdata<-filter(xdata, plot %in% snowplots$plot)
ydata <- filter(spp_abundw,plot %in% snowplots$plot)%>%
...
plothits_snow<-filter(plothits, plot %in% snowplots$plot)
vals=plothits_snow$veg_hits
...
modDAtime_snow <- gjam(formula=timeList$formulaRho, xdata = xdata, ydata = ydata, modelList = modelList)

# save output
save(modDAtime_snow, file = "outputs/modDAtime_snowoutput.RData")
...
plotPars <- list(specColor=specColor, PLOTALLY=T, GRIDPLOTS=T, CLUSTERPLOTS=T, SAVEPLOTS = T, 
                 outFolder = 'plots/modDAtime_snowplots')
gjamPlot(modDAtime_snow, plotPars)

#spp alphas
alphaX<-modDAtime_snow$parameters$alphaMu
colnames(alphaX)<-colnames(ydata)
row.names(alphaX)<-colnames(alphaX)  

pdf(file="plots/modDAtime_snowplots/alpha_plot.pdf")
...
corr<-modDAtime_snow$parameters$corMu
colnames(corr)<-colnames(ydata)
row.names(corr)<-colnames(corr)  

pdf(file="plots/modDAtime_snowplots/corr_plot.pdf")
...
wstarsnow <- .wrapperEquilAbund(output =   modDAtime_snow, covars = c('depthcm', 'avgT', 'Ndep'), BYGROUP = F,
                               nsim = 100, ngrid=10,
                               verbose = T)
save(wstarsnow, file = "outputs/wstar_snowoutput.RData")
#load(file = "outputs/wstar_snowoutput.RData")
...
#subset data for snow addition plots
ctlplots<-subset(sppcomp, snow=="X")  
xdata<-filter(xdata, plot %in% ctlplots$plot)
ydata <- filter(spp_abundw,plot %in% ctlplots$plot)%>%
  select(-plotyear, -N, -snow, -temp, -plot, -block, -year) 
ydata$year <- NULL
ydata$plot <- NULL
ydata <- as.matrix(ydata)
...
plothits<-select(spp_abund%>%ungroup(), veg_hits, plotyear)%>%distinct(.)
plothits<-separate(plothits, col = plotyear,into = c("plot","year"), sep = "_", remove=F)#ignore warning 
plothits_ctl<-filter(plothits, plot %in% ctlplots$plot)
vals=plothits_ctl$veg_hits
edata <- matrix(vals, nrow = nrow(xdata), ncol = ncol(ydata))
...
modDAtime_ctlsnow <- gjam(formula=timeList$formulaRho, xdata = xdata, ydata = ydata, modelList = modelList)

# save output
save(modDAtime_ctlsnow, file = "outputs/modDAtime_ctlsnowoutput.RData")
...
#plotPars1 <- list(specColor=specColor, PLOTALLY=T, GRIDPLOTS=T, CLUSTERPLOTS=T, SAVEPLOTS = F)
plotPars <- list(specColor=specColor, PLOTALLY=T, GRIDPLOTS=T, CLUSTERPLOTS=T, SAVEPLOTS = T, 
                 outFolder = 'plots/modDAtime_ctlsnowplots')
gjamPlot(modDAtime_ctlsnow, plotPars)

#spp alphas
alphaX<-modDAtime_ctlsnow$parameters$alphaMu
colnames(alphaX)<-colnames(ydata)
row.names(alphaX)<-colnames(alphaX)  

pdf(file="plots/modDAtime_ctlsnowplots/alpha_plot.pdf")
corrplot(alphaX ,method = "color", tl.cex = 0.8, tl.col="black", addCoef.col = "black",
         number.cex = 0.75, diag =T, main="alphas",  is.corr = F,
         mar = c(2, 2, 2, 2), cl.lim = c(-1, 0))
dev.off()

corr<-modDAtime_ctlsnow$parameters$corMu
colnames(corr)<-colnames(ydata)
row.names(corr)<-colnames(corr)  

pdf(file="plots/modDAtime_ctlsnowplots/corr_plot.pdf")
corrplot(corr, method = "color", tl.cex = 0.8, tl.col="black", addCoef.col = "black",
         number.cex = 0.75, diag = F, main="correlations" ,
         mar = c(2, 2, 2, 2))  
dev.off()
...
#calculate equillibrium abundance 
wstarctlsnow <- .wrapperEquilAbund(output =   modDAtime_ctlsnow, covars = c('depthcm', 'avgT', 'Ndep'), BYGROUP = F,
                                nsim = 100, ngrid=10,
                                verbose = T)
save(wstarctlsnow, file = "outputs/wstar_ctlsnowoutput.RData")
#load(file = "outputs/wstar_ctlsnowoutput.RData")
