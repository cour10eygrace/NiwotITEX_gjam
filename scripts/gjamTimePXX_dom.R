...
# save output
save(modDAtimePXX,file = "outputs/modDAtime_PXXoutput_dom.RData")
...
plotPars <- list(specColor=specColor, PLOTALLY=T, GRIDPLOTS=T, CLUSTERPLOTS=T, SAVEPLOTS = T, 
                 outFolder = 'plots/modDAtime_PXXplots_dom')
...
pdf(file="plots/modDAtime_PXXplots_dom/alpha_plot.pdf")
...
pdf(file="plots/modDAtime_PXXplots_dom/corr_plot.pdf")
...
wstarPXX <- .wrapperEquilAbund(output =   modDAtimePXX, covars = c('depthcm', 'avgT', 'Ndep'), BYGROUP = F,
                               nsim = 100, ngrid = 10,
                               verbose = T)
save(wstarPXX, file = "outputs/wstar_PXXoutput_dom.RData")
#load(file = "outputs/wstar_PXXoutput_dom.RData")
