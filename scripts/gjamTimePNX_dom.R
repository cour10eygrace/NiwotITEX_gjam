...
# save output
save(modDAtimePNX,file = "outputs/modDAtime_PNXoutput_dom.RData")
...
plotPars <- list(specColor=specColor, PLOTALLY=T, GRIDPLOTS=T, CLUSTERPLOTS=T, SAVEPLOTS = T, 
                 outFolder = 'plots/modDAtime_PNXplots_dom')
...
pdf(file="plots/modDAtime_PNXplots_dom/alpha_plot.pdf")
...
pdf(file="plots/modDAtime_PNXplots_dom/corr_plot.pdf")
...
wstarPNX <- .wrapperEquilAbund(output =   modDAtimePNX, covars = c('depthcm', 'avgT', 'Ndep'), BYGROUP = F,
                               nsim = 100,ngrid=10, 
                               verbose = T)
save(wstarPNX, file = "outputs/wstar_PNXoutput_dom.RData")
#load(file = "outputs/wstar_PNXoutput_dom.RData")
