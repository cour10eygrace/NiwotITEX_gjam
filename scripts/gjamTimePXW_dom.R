...
spp_abundw<-select(spp_abundw, DESCAE, SUBDOM, MODERATE, RARE, year, plot)%>%rename(DOM=DESCAE)# doesn't like underscores in spp names
...
# save output
save(modDAtimePXW,file = "outputs/modDAtime_PXWoutput_dom.RData")
...
plotPars <- list(specColor=specColor, PLOTALLY=T, GRIDPLOTS=T, CLUSTERPLOTS=T, SAVEPLOTS = T, 
                 outFolder = 'plots/modDAtime_PXWplots_dom')
...
pdf(file="plots/modDAtime_PXWplots_dom/alpha_plot.pdf")
...
pdf(file="plots/modDAtime_PXWplots_dom/corr_plot.pdf")
...
wstarPXW <- .wrapperEquilAbund(output =   modDAtimePXW, covars = c('depthcm', 'avgT', 'Ndep'), BYGROUP = F,
                               nsim = 100, ngrid = 10,
                               verbose = T)
save(wstarPXW, file = "outputs/wstar_PXWoutput_dom.RData")
