...
# save output
save(modDAtimeXNX, file = "outputs/modDAtime_XNXoutput_dom.RData")
...
plotPars <- list(specColor=specColor, PLOTALLY=T, GRIDPLOTS=T, CLUSTERPLOTS=T, SAVEPLOTS = T, 
                 outFolder = 'plots/modDAtime_XNXplots_dom')
...
pdf(file="plots/modDAtime_XNXplots_dom/alpha_plot.pdf")
...
pdf(file="plots/modDAtime_XNXplots_dom/corr_plot.pdf")
...
wstarXNX <- .wrapperEquilAbund(output =   modDAtimeXNX, covars = c('depthcm', 'avgT', 'Ndep'), BYGROUP = F,
                               nsim = 100, ngrid=10, 
                               verbose = T)
save(wstarXNX, file = "outputs/wstar_XNXoutput_dom.RData")
#load(file = "outputs/wstar_XNXoutput_dom.RData")
